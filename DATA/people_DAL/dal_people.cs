...
namespace SnitchNet_PROJECT_9_6_25.DATA.people_DAL
{
    internal class dal_people
    {
        ...
        public static void add_people(string firstName, string lastName, string secret_code, string type = "clean", int num_mentions =0, int num_reports = 0)
        {
...
'{new_people.CreatedAt:yyyy-MM-dd HH:mm:ss}')";
        ...
        WHERE LOWER(FirstName) = LOWER('{firstname}') 
          AND LOWER(LastName) = LOWER('{last_name}') 
          AND secret_code = '{code_name}'";
            }
            else
            {
                sql = $@"
